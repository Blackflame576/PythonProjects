...
async def HdCompress(call: types.CallbackQuery):
    ...
    if os.path.exists(PathVideo + f'/{video.default_filename}'):
        ...
        elif not id_user in VideosForCompress:
    ...
    elif not os.path.exists(PathVideo + f'/{video.default_filename}'):
        await bot.send_message(chat_id = id_user,text = f"Download video '{video.default_filename}'")
        download = video.download(PathVideo)
        ...
        if id_user in VideosForCompress:
            VideosForCompress[id_user].append(download)
        elif not id_user in VideosForCompress:
            VideosForCompress.update({id_user:[download]})
...
async def UploadVideo():
    ...
    try:
        ...
        for User in CompressedVideos:
            for NumberVideo,Video in enumerate(CompressedVideos[User]):
                Video = Video
                User = User
                if CompressedVideos[User][NumberVideo] != None: 
